...
BEGIN_MESSAGE_MAP(CInterfaceDlg, CDialogEx)
	...
	ON_CONTROL_RANGE(BN_CLICKED, IDC_GLCM, IDC_LBP, &CInterfaceDlg::OnBnClickedWeight)
END_MESSAGE_MAP()
...
BOOL CInterfaceDlg::OnInitDialog()
{
	...
	pMQueryTime = (CEdit *)GetDlgItem(IDC_MQUERYTIME);
	...
	pWLBP = (CEdit *)GetDlgItem(IDC_EDIT_LBP);
...
void CInterfaceDlg::OnBnClickedIndex()
{
	...
	else if(find.FindFile(L"E:\\feat.dat"))
	{
		...
		for(int i = 0; i < TOTALIMG; i++)
			inf.read((char *)&features[i], sizeof(ImageFeature));
		inf.close();
		...
		pdmaxLBP = new double[LBP_length];
		pdminLBP = new double[LBP_length];
...
void CInterfaceDlg::Normalization()
	...
	for(int i = 0; i < TOTALIMG; i++)
	{
		...
		{
			if(features[i].Sift[j] > pdmaxSIFT[j])
				pdmaxSIFT[j] = features[i].Sift[j];
			if(features[i].Sift[j] < pdminSIFT[j])
				pdminSIFT[j] = features[i].Sift[j];
		}
		...
		{
			if(features[i].Lbp[j] > pdmaxLBP[j])
				pdmaxLBP[j] = features[i].Lbp[j];
			if(features[i].Lbp[j] < pdminLBP[j])
...
void CInterfaceDlg::StoreFeatures()
{
	...
	for(int i = 0; i < TOTALIMG; i++)
		of.write((char *)&features[i], sizeof(ImageFeature));
	of.close();
...
void CInterfaceDlg::OnBnClickedRand200()
{
	...
	else
	{
		...
		for(int i = 0; i < 200; i++)
		{
			for(int j = 0; j < GLCM_length && ischeck[GLCM]; j++)
			{
				if(pdmaxGLCM[j] - pdminGLCM[j] == 0)
			...
			for(int j = 0; j < EH_length && ischeck[EH]; j++)
			{
				if(pdmaxEH[j] - pdminEH[j] == 0)
			...
			//特征有可能出现负值
			for(int j = 0; j < HU_length && ischeck[HU]; j++)
			{
				if(tfeat[i].Hu[j] > 0)
				{
			...
			//特征有可能出现负值
			for(int j = 0; j < HSV_length && ischeck[HSV]; j++)
			{
				if(pdmaxHSV[j] - pdminHSV[j] == 0)
					tfeat[i].HSVFeat[j] = 0;
				else
			...
			for(int j = 0; j < SIFT_length && ischeck[SIFT]; j++)
			{
				if(pdmaxSIFT[j] - pdminSIFT[j] == 0)
					tfeat[i].Sift[j] = 0;
				else
				{
					tfeat[i].Sift[j] = (tfeat[i].Sift[j] - pdminSIFT[j]) / (pdmaxSIFT[j] - pdminSIFT[j]);
					if(tfeat[i].Sift[j] < 0)
						tfeat[i].Sift[j] = 0;
				}
			}
			for(int j = 0; j < WAVE_length && ischeck[WAVELET]; j++)
			{
				if(pdmaxWAVE[j] - pdminWAVE[j] == 0)
			...
			for(int j = 0; j < LBP_length && ischeck[LBP]; j++)
			{
				if(pdmaxLBP[j] - pdminLBP[j] == 0)
					tfeat[i].Lbp[j] = 0;
				else
				{
					tfeat[i].Lbp[j] = (tfeat[i].Lbp[j] - pdminLBP[j]) / (pdmaxLBP[j] - pdminLBP[j]);
					if(tfeat[i].Lbp[j] < 0)
						tfeat[i].Lbp[j] = 0;
				}
			}
		...
		t_end = clock();
		TT += ((t_end - t_start + 0.0) / CLOCKS_PER_SEC);
...
void CInterfaceDlg::OnBnClickedWeight(UINT nid)
{
	...
	case IDC_LBP:
		{
			if(pCheckLBP->GetCheck())
				pWLBP->SetWindowTextW(L"1");
			else
				pWLBP->SetWindowTextW(L"0");
			break;
		}
	default:
		break;
